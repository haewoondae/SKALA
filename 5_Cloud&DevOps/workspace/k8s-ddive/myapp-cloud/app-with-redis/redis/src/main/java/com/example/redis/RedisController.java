...
public class RedisController {
    ...
    public ResponseEntity<String> setValue(@RequestParam String key, @RequestParam String value) {
        try {
            String result = redisService.setValue(key, value);
            return ResponseEntity.ok("키 '" + key + "'에 값 '" + value + "' 저장 완료: " + result);
        } catch (Exception e) {
    ...
    public ResponseEntity<String> setValueJson(@RequestBody KeyValueRequest request) {
        try {
            String result = redisService.setValue(request.getKey(), request.getValue());
            return ResponseEntity.ok("키 '" + request.getKey() + "'에 값 '" + request.getValue() + "' 저장 완료: " + result);
        } catch (Exception e) {
            return ResponseEntity.status(500).body("에러 발생: " + e.getMessage());
        }
    }
    ...
    public ResponseEntity<String> deleteKey(@PathVariable String key) {
        try {
            Long deletedCount = redisService.deleteKey(key);
            if (deletedCount > 0) {
